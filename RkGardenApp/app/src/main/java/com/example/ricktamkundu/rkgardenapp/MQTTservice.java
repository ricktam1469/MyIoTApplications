	...
	private class MQTTConnection extends Thread
	{
		...
		private class MsgHandler extends Handler implements MqttCallback
		{
			...
			public void messageArrived(String topic, MqttMessage message) throws Exception
			{
				...
				if(message.toString().contains("Moisture")) {
					Builder notificationCompat1 = new Builder(context);
					notificationCompat1.setAutoCancel(true)
							.setContentIntent(pendingIntent)
							.setContentText("GardenApp: " + message.toString())
							.setAutoCancel(true)
							.setColor(Color.BLUE)
							.setSmallIcon(R.mipmap.ic_launcher)
							.setSound(uri);

					Notification notification1 = notificationCompat1.build();
					NotificationManager nm1 = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
					nm1.notify(mid, notification1);
				}
				if(message.toString().contains("Motor")) {
				Builder notificationCompat2 = new Builder(context);
				notificationCompat2.setAutoCancel(true)
						.setContentIntent(pendingIntent)
						.setContentText("GardenApp: " + message.toString())
						.setAutoCancel(true)
						.setColor(Color.BLUE)
				...
				Notification notification2 = notificationCompat2.build();
				NotificationManager nm2 = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
				nm2.notify(mid+1, notification2);
				}

				if(message.toString().contains("Tank")) {
					Builder notificationCompat3 = new Builder(context);
					notificationCompat3.setAutoCancel(true)
							.setContentIntent(pendingIntent)
							.setContentText("GardenApp: " + message.toString())
							.setAutoCancel(true)
							.setColor(Color.BLUE)
